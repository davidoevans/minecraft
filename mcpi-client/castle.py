...
def CreateWalls(x, y, z, size, height, material, battlements, walkway):
    ...
    # Add wooden walkways
    if walkway == True:
        mc.setBlocks(x - size + 1, y + height - 1, z + size - 1, x + size - 1, y + height - 1, z + size - 1,
                     block.WOOD_PLANKS)
        mc.setBlocks(x - size + 1, y + height - 1, z - size + 1, x + size - 1, y + height - 1, z - size + 1,
                     block.WOOD_PLANKS)
        mc.setBlocks(x - size + 1, y + height - 1, z - size + 1, x - size + 1, y + height - 1, z + size - 1,
...
def CreateLandscape(x, y, z, islandwidth, moatwidth, moatdepth):
    ...
    # Create water moat
    mc.setBlocks(x - islandwidth - moatwidth, y, z - islandwidth - moatwidth, x + islandwidth + moatwidth,
                 y - moatdepth, z + islandwidth + moatwidth, block.WATER)
    # Create island
...
def CreateKeep(x, y, z, size, floors):
    ...
    for floor in range(1, floors + 1):
        mc.setBlocks(x - size + 1, (floor * 5) + y, z - size + 1, x + size - 1, y + (floor * 5), z + size - 1,
                     block.WOOD_PLANKS)
